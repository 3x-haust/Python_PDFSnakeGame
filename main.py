...
// Snake specific globals
var snake = [{x: 7, y: 7}, {x: 6, y: 7}, {x: 5, y: 7}]; // Initial snake position
var direction = {x: 1, y: 0}; // Moving right initially
var nextDirection = {x: 1, y: 0};
var food = null;

var TICK_INTERVAL = 50;
var GAME_STEP_TIME = 200; // Faster than Tetris for better control

function spawn_food() {
    do {
        var x = Math.floor(rand() % ###GRID_WIDTH###);
        var y = Math.floor(rand() % ###GRID_HEIGHT###);
        // Check if position is free (not occupied by snake)
        var occupied = false;
...
function handle_input(event) {
    switch (event.change) {
        case 'w': // Up
            if (direction.y !== 1) { // Can't move up if moving down
                nextDirection = {x: 0, y: -1};
            }
            break;
        case 's': // Down
            if (direction.y !== -1) { // Can't move down if moving up
                nextDirection = {x: 0, y: 1};
            }
            break;
        case 'a': // Left
            if (direction.x !== 1) { // Can't move left if moving right
                nextDirection = {x: -1, y: 0};
            }
            break;
        case 'd': // Right
            if (direction.x !== -1) { // Can't move right if moving left
                nextDirection = {x: 1, y: 0};
            }
            break;
    }
...
add_button("^", "B_up", GRID_OFF_X + 60, GRID_OFF_Y - 70, 50, 50, "move_up();")
add_button("<", "B_left", GRID_OFF_X + 0, GRID_OFF_Y - 130, 50, 50, "move_left();")
add_button(">", "B_right", GRID_OFF_X + 120, GRID_OFF_Y - 130, 50, 50, "move_right();")
add_button("\\\\/", "B_down", GRID_OFF_X + 60, GRID_OFF_Y - 130, 50, 50, "move_down();")
